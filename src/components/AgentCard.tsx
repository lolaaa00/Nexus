...
const AgentCard = ({ agent, index, onSelect }: AgentCardProps) => {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: index * 0.1, duration: 0.4 }}
      className="glass-card-solid rounded-3xl p-8 glow-hover cursor-pointer group"
    ...
    >
      ...
      <button
        ...
        className="w-full bg-primary text-primary-foreground py-3 rounded-xl font-medium text-sm hover:opacity-90 transition-opacity"
      >
        Run Agent
      </button>
    </motion.div>
